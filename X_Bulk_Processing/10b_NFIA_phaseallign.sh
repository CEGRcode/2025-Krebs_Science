...
for  XX in RR YY WW SS ; do
        ...
        cat $DIRQ1/10xCDT/${XX}_${Ref}_nearNuc_original_phase_8_sense.cdt | cut -f  1-2,251-750 | tail -n +2 >  $DIR/${XX}_${Ref}_nearNuc_phase_8_sense.cdt
        cat $DIRQ1/10xCDT/${XX}_${Ref}_nearNuc_original_phase_7_sense.cdt | cut -f  1-2,250-749 | tail -n +2 >  $DIR/${XX}_${Ref}_nearNuc_phase_7_sense.cdt
        cat $DIR/${XX}_${Ref}_nearNuc_phase_0_sense.cdt $DIR/${XX}_${Ref}_nearNuc_phase_1_sense.cdt $DIR/${XX}_${Ref}_nearNuc_phase_9_sense.cdt $DIR/${XX}_${Ref}_nearNuc_phase_8_sense.cdt $DIR/${XX}_${Ref}_nearNuc_phase_7_sense.cdt > $DIR/${XX}_${Ref}_nearNuc_phase_prefered_sense.cdt
        rm $DIR/${XX}_${Ref}_nearNuc_phase_0_sense.cdt $DIR/${XX}_${Ref}_nearNuc_phase_1_sense.cdt $DIR/${XX}_${Ref}_nearNuc_phase_9_sense.cdt $DIR/${XX}_${Ref}_nearNuc_phase_8_sense.cdt $DIR/${XX}_${Ref}_nearNuc_phase_7_sense.cdt
done
...
mkdir -p $DIR/NFIA_phase_prefered
mv $DIR/*232_sense.cdt  $DIR/NFIA_phase_prefered


for file in $DIR/NFIA_phase_prefered/*_phase_prefered_*_sense.cdt  ; do
      filename=$(basename "$file" ".cdt")
      java -jar $SCRIPTMANAGER read-analysis aggregate-data --sum $file -o $DIR/NFIA_phase_prefered/${filename}_SCORES.out 
      tail -n +2 $DIR/NFIA_phase_prefered/${filename}_SCORES.out  > $DIR/NFIA_phase_prefered/${filename}_score.out
      ...
      awk  -v max_col=200 '{
        ...
        }'  | paste $DIR/NFIA_phase_prefered/${filename}_score.out  - |  awk '{
    OFS="\t";
    print $1,$2,$3/($2+1),$4/($2+1),$5/($2+1),$6/($2+1),$7/($2+1),$8/($2+1),$9/($2+1),$10/($2+1),$11/($2+1),$12/($2+1)}' > $DIR/NFIA_phase_prefered/${filename}_score_peak.out
    rm $DIR/NFIA_phase_prefered/${filename}_SCORES.out 
    rm $DIR/NFIA_phase_prefered/${filename}_score.out
done
